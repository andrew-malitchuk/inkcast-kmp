 ...
 *
 * Handles both CrossPoint and CrossPet firmware:
 * - CrossPoint: READY → binary chunks → DONE/ERROR
 * - CrossPet: READY → binary chunks → PROGRESS:<received>:<total> (every 64 KB) → DONE/ERROR
 *
        ...
        repeat(3) { attempt ->
            ...
            try {
                ...
                client.webSocket(host = cleanIp, port = 81, path = "/") {
                    ...
                    // NOTE: Listener runs in parallel to detect READY/DONE/ERROR/PROGRESS signals
                            ...
                            for (frame in incoming) {
                                if (frame is Frame.Text) {
                                    ...
                                    when {
                                        ...
                                        // NOTE: CrossPet firmware sends PROGRESS:<received>:<total>
                                        // every 64 KB to report device-side write progress.
                                        // CrossPoint does not send this frame — handled as no-op there.
                                        text.startsWith("PROGRESS:") -> {
                                            val parts = text.removePrefix("PROGRESS:").split(":")
                                            if (parts.size == 2) {
                                                val received = parts[0].toLongOrNull() ?: return@launch
                                                val total = parts[1].toLongOrNull()?.takeIf { it > 0 } ?: return@launch
